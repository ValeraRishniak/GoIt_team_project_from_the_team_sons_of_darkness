    ...
    if path_folder.exists():
        for f in path_folder.iterdir():
            try:
                ...
                print(f"WARNING. {f.name} не може бути розархівованим.")
            finally:
                continue
    else:
        pass
...
def main():
    way = input(f'Вкажіть шлях до теки: ')
    print(f'Починаю сортувати файли у теці: {str(way)}')
    # try:
    #     path = Path(way, sys.argv[1])
    #     print(f'Починаю сортувати файли у теці: {way}')
    # except IndexError:
    #     print("Будь-ласка введіть корректний шлях до теки.")

    create_folders(Path(way))
    sort_files(Path(way))
    delete_folders(Path(way))
    unpack_archives(Path(way))
